...
<template>
    <div class="tags-view-container">
        <ScrollPane>
            <router-link class="tags-view-item" :class="{ active: isActive(tag) }" v-for="(tag, index) in visitedViews "
                :to="{ path: tag.path, query: tag.query, fullPath: tag.fullPath }" :key="tag.path">
                <span class="circle" v-if="isActive(tag)"></span>
                {{ tag.title }}
                <el-icon v-if="!isAffix(tag)" @click.prevent.stop="close(index)">
                    <Close />
                </el-icon>
            </router-link>
        </ScrollPane>
...
function GoToLastView() {
    const { path, query } = tagsView.visitedViews[tagsView.visitedViews.length - 1];
